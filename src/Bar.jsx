...
const barApps = [
  ['Tarayıcı',        'tarayıcı',  'Chromium tabanlı, reklam engelleyici', '⌂'],
  ['Medya Oynatıcı',  'medya',     'Tüm formatlar, Türkçe altyazı',        '►'],
  ['Dosya Yöneticisi','dosyalar',  'Türkçe komutlarla dosya yönetimi',     '⎘'],
  ['Hesap Makinesi',  'hesap',     'Bilimsel + finansal mod',              '#'],
  ['Not Defteri',     'not',       'Basit, hızlı, çevrimdışı',             '⎙'],
  ['Ayarlar',         'ayarlar',   'Sistem tercihleri',                    '◎'],
  ['Güvenlik',        'güvenlik',  'Güncellemeler, güvenlik duvarı',       '⏚'],
  ['Yardım',          'yardım',    'Türkçe kılavuz',                       '?'],
];

const barNot = ['Uçbirim','Ofis paketi','Paket yöneticisi','Geliştirme araçları','Oyun merkezi','Sosyal medya'];
  ...
  return (
    <>
      <PageHead
        ...
        lede="Hiçbir gereksiz uygulama, hiçbir dikkat dağıtıcı. Temiz bir masaüstü, hızlı bir sistem, sade bir deneyim."
        ornamentColor="var(--jade)"
      >
        <div style={{display:'flex', gap:12, marginTop:36, flexWrap:'wrap', alignItems:'center'}}>
          <Link to="/hakkinda" className="btn btn-crimson" style={{background:'var(--jade)', boxShadow:'0 12px 30px -10px rgba(63,142,99,0.4)'}}>Bar sürümünü indir →</Link>
      ...
      <section className="section--loose section" style={{position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{maxWidth:920, textAlign:'center', margin:'0 auto', position:'relative', zIndex:2}}>
          ...
          <p className="lede reveal delay-1" style={{marginInline:'auto', marginTop:44}}>
            ...
            sunar. 2 GB RAM ile çalışır, 15 saniyede açılır, pil ömrü diğer sürümlere göre
            %40 daha uzundur.
          </p>
          ...
          <div className="reveal delay-2" style={{
            ...
            {[
              ...
              <div key={l} style={{
                ...
                <div style={{fontFamily:'var(--display)', fontSize:'clamp(48px, 5vw, 80px)', lineHeight:1, color:'var(--ink)'}}>{n}</div>
                <div className="eyebrow" style={{marginTop:16, color:'var(--jade)'}}>{l}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="reveal" style={{
            ...
            {barApps.map((a, i) => (
              <div key={a[0]} style={{
                ...
                <h4 style={{marginTop:24, fontSize:17, color:'var(--ink)', fontWeight:500}}>{a[0]}</h4>
                <p style={{fontSize:13, color:'var(--sand)', marginTop:8, minHeight:'3em'}}>{a[2]}</p>
                <code style={{display:'block', marginTop:14, fontFamily:'var(--mono)', fontSize:12, color:'var(--jade)'}}>{a[1]}</code>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section" style={{background:'var(--ink-1)', borderTop:'1px solid var(--ink-line)', borderBottom:'1px solid var(--ink-line)', position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{position:'relative', zIndex:2}}>
          ...
          <div className="grid grid-3 reveal" style={{marginTop:64}}>
            {[
              ['Kiosk sistemleri',  'ATM\'ler, bilgi ekranları, self-servis terminaller.', '⌧'],
              ['Ev kullanıcıları',  'İnternet, e-posta, medya — temel ihtiyaçlar.', '⌂'],
              ['Eski bilgisayarlar','10+ yıllık donanımda bile hızlı çalışır.', '◉'],
            ].map(([t, b, icon], i) => (
              <div key={t} className="card card--jade">
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline'}}>
                  <span className="mono" style={{color:'var(--jade)'}}>№ 0{i+1}</span>
                  <span style={{fontFamily:'var(--display)', fontSize:32, color:'var(--jade)', lineHeight:1}}>{icon}</span>
                </div>
                <h3 className="h3" style={{marginTop:24}}>{t}</h3>
      ...
      <section className="section" style={{position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{textAlign:'center', position:'relative', zIndex:2}}>
          ...
          <div className="reveal delay-2" style={{display:'flex', gap:12, justifyContent:'center', marginTop:36, flexWrap:'wrap'}}>
            <Link to="/hakkinda" className="btn btn-crimson" style={{background:'var(--jade)', boxShadow:'0 12px 30px -10px rgba(63,142,99,0.4)'}}>ISO indir (1.4 GB) →</Link>
          </div>
          <div className="eyebrow reveal delay-3" style={{marginTop:28}}>Minimum 2 GB RAM · 15 GB disk</div>
        </div>
      </section>
    </>
  );
