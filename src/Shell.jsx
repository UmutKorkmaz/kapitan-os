/* Shared shell: routing, Nav, Footer, helpers */
...
function readInitialTheme() {
  ...
  // Default: respect user OS preference; if neither, light (the new canonical)
  ...
  return (
    ...
    >
      <span className="theme-toggle__face" aria-hidden="true">
        {/* Sun (visible in dark theme → click to go light) */}
        ...
        {/* Moon (visible in light theme → click to go dark) */}
...
function Nav({ activePath, onOpenCmdK }) {
  return (
    <header className="nav">
      <div className="wrap nav-inner">
        <Link to="/" className="nav-brand" aria-label="KAPiTaN OS">
          ...
          <span>KAPiTaN <i>OS</i></span>
          <b>v3.2 · TR</b>
        </Link>
        <nav className="nav-links">
          {NAV_LINKS.map(l => (
            <Link key={l.to} to={l.to}
              className={activePath?.startsWith(l.to) ? 'active' : ''}>
              {l.label}
            </Link>
          ))}
        </nav>
        <div className="nav-right">
          <button className="nav-cmd" onClick={onOpenCmdK} title="Komut paleti">
            <span>komut ara…</span>
            <kbd>⌘K</kbd>
          </button>
          <ThemeToggle />
          <Link to="/hakkinda" className="btn btn-crimson nav-cta">İndir →</Link>
        </div>
      </div>
    </header>
  );
        ...
        <div className="footer-grid">
          <div>
            ...
            <div style={{display:'flex', gap:8, marginTop:24, flexWrap:'wrap'}}>
              <span className="pill"><span className="ldot" /> KARARLI · v3.2.4</span>
              <span className="pill"><span className="ldot" style={{background:'var(--saffron)'}}/> 2026/05</span>
            </div>
          </div>
          <div>
            <h4>Sürümler</h4>
            <ul>
              <li><Link to="/surumler/gelistirici">Geliştirici</Link></li>
              <li><Link to="/surumler/ofis">Ofis</Link></li>
              <li><Link to="/surumler/bar">Bar (minimal)</Link></li>
              <li><Link to="/surumler">Karşılaştır</Link></li>
            </ul>
          </div>
          <div>
            <h4>Kaynaklar</h4>
            <ul>
              <li><Link to="/komutlar">Komut rehberi</Link></li>
              <li><Link to="/belgeler">Belgeler</Link></li>
              <li><Link to="/pazar">KAPiTaN Pazar</Link></li>
              <li><Link to="/topluluk">Forum</Link></li>
            </ul>
          </div>
          <div>
            <h4>Proje</h4>
            <ul>
              ...
              <li><Link to="/topluluk">Topluluk</Link></li>
              <li><Link to="/hakkinda">Kaynak kodu</Link></li>
              <li><Link to="/hakkinda">İletişim</Link></li>
            </ul>
          </div>
        </div>
...
function PageHead({ crumbs, title, lede, children, ornamentColor = 'var(--crimson)' }) {
  return (
...
/* ---------- Section head ---------- */
function SectionHead({ no, eyebrow, title, lede, align }) {
  return (
    <div className="section-head" style={{textAlign: align || 'left', maxWidth: align === 'center' ? 780 : 'none', marginInline: align === 'center' ? 'auto' : 0}}>
      ...
      {lede && <p className="lede" style={{marginTop:24, marginInline: align === 'center' ? 'auto' : 0}}>{lede}</p>}
    </div>
  );
}

/* ---------- Section ornament divider ---------- */
function Divider({ color = 'var(--crimson)' }) {
  return (
...
function Term({ title, lines, height }) {
  return (
...
Object.assign(window, {
  useHashRoute, useReveal, Link,
  Nav, Footer, PageHead, SectionHead, Divider, Term, ScrollProgress
});
