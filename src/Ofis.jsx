...
const ofisSuite = [
  { name:'KelimeIşlem Pro',  desc:'Türkçe imla denetimi, 50+ şablon. DOCX, ODT uyumlu. AI özetleme ve çeviri.', cmd:'belge', icon:'¶' },
  { name:'HesapTablosu Pro', desc:'Formüller Türkçe: TOPLA, ORTALAMA, EĞER. Grafik, pivot, XLSX uyumlu.', cmd:'tablo', icon:'∑' },
  { name:'SunumAracı',       desc:'30+ Türkçe tema, geçiş, animasyon. PPTX uyumlu. AI taslak.', cmd:'sunum', icon:'◉' },
  { name:'NotDefteri',       desc:'AI kategorilendirme, sesli not (Türkçe), senkron.', cmd:'not', icon:'⎙' },
];

const ofisApps = [
  ['KelimeIşlem', '¶'],['HesapTablosu', '∑'],['SunumAracı','◉'],['EpostaYöneticisi','@'],
  ['Takvim','⌛'],['NotDefteri','⎙'],['HesapMakinesi','#'],['Tarayıcı','⌂'],
  ['Sözlük','Α'],['PDFOkuyucu','▤'],['ResimGörüntü','◇'],['MüzikÇalar','♪'],
];
  ...
  return (
    <>
      <PageHead
        ...
        lede="Tam entegre ofis paketi, akıllı e-posta yönetimi, yapay zekâ yazı asistanı — hepsi Türkçe arayüzde."
        ornamentColor="var(--saffron)"
      >
        <div style={{display:'flex', gap:12, marginTop:36, flexWrap:'wrap', alignItems:'center'}}>
          <Link to="/hakkinda" className="btn btn-crimson" style={{background:'var(--saffron)', color:'var(--ink)', boxShadow:'0 12px 30px -10px rgba(232,178,62,0.4)'}}>Ofis sürümünü indir →</Link>
            ...
            {ofisSuite.map((a, i) => (
              <div key={a.name} className="card card--saffron" style={{padding:'40px', position:'relative', overflow:'hidden'}}>
                ...
                <div style={{position:'relative', zIndex:2}}>
                  ...
                  <code style={{display:'inline-block', marginTop:18, padding:'6px 12px', background:'var(--ink)', border:'1px solid var(--ink-line)', borderRadius:3, fontFamily:'var(--mono)', fontSize:12, color:'var(--saffron)'}}>
                    <span style={{color:'var(--crimson)', marginRight:8}}>›</span>{a.cmd}
                  </code>
                </div>
              </div>
            ))}
        ...
        <div className="wrap">
          <div className="reveal">
            <SectionHead no="04" eyebrow="Önyüklü uygulamalar"
              title={<><em style={{color:'var(--saffron)'}}>30+</em> uygulama, hepsi Türkçe.</>}
            />
          </div>
          <div className="reveal" style={{
            ...
            {ofisApps.map(([name, icon], i) => (
              <div key={name} style={{
                ...
                <div>
                  <div style={{fontFamily:'var(--display)', fontSize:40, lineHeight:1, color:'var(--saffron)'}}>{icon}</div>
                  <div style={{fontSize:12, color:'var(--sand)', marginTop:8}}>{name}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section" style={{position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{textAlign:'center', position:'relative', zIndex:2}}>
          ...
          <div className="reveal delay-2" style={{display:'flex', gap:12, justifyContent:'center', marginTop:36, flexWrap:'wrap'}}>
            <Link to="/hakkinda" className="btn btn-crimson" style={{background:'var(--saffron)', color:'var(--ink)', boxShadow:'0 12px 30px -10px rgba(232,178,62,0.4)'}}>ISO indir (3.1 GB) →</Link>
          </div>
          <div className="eyebrow reveal delay-3" style={{marginTop:28}}>Minimum 4 GB RAM · 30 GB disk</div>
        </div>
      </section>
    </>
  );
