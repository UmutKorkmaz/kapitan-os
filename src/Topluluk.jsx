/* Topluluk — community with human warmth */
...
const tpStats = [
  ['52 318', 'Aktif kullanıcı'],
  ['1 247',  'Katkıda bulunan'],
  ['218',    'Komut çevirisi'],
  ['38',     'Yerelleştirme dili'],
];
  ...
  return (
    <>
      <PageHead
        ...
        title={<>Türkçe yazılım <em>topluluğu</em>.</>}
        lede="KAPiTaN OS bir şirket değil; on binlerce gönüllünün katkısıyla yürüyen açık kaynak bir projedir. Forum, etkinlikler ve katkı yolları aşağıda."
        ornamentColor="var(--saffron)"
      >
        <div style={{display:'flex', gap:12, marginTop:36, flexWrap:'wrap'}}>
          <a href="#join" className="btn btn-crimson">Foruma katıl →</a>
          <a href="#contrib" className="btn btn-line">Katkıda bulun</a>
        </div>
      </PageHead>
          ...
          <div className="reveal" style={{
            ...
            {tpStats.map(([n, l], i) => (
              <div key={l} style={{
                ...
                <div className="eyebrow" style={{marginTop:16, color:'var(--saffron)'}}>{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section" style={{background:'var(--ink-1)', borderTop:'1px solid var(--ink-line)', borderBottom:'1px solid var(--ink-line)', position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{position:'relative', zIndex:2}}>
          <div className="reveal" style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:16}}>
            <SectionHead no="01" eyebrow="Yaklaşan etkinlikler"
              title={<>Yan yana, <em>yüz yüze</em>.</>}
            />
            <a href="#" className="tlink">Tüm takvim →</a>
          </div>

          <div className="reveal" style={{marginTop:64, border:'1px solid var(--ink-line)', borderRadius:8, overflow:'hidden', background:'var(--ink-2)'}}>
            {tpEvents.map((ev, i) => {
              const pct = (ev.spots / ev.total) * 100;
              return (
                <a key={ev.t} href="#" style={{
                  display:'grid',
                ...
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  ...
                  <div>
                    ...
                    <div className="mono" style={{fontSize:11, letterSpacing:'0.18em', color:ev.acc, marginTop:6}}>{ev.m} 2026</div>
                  </div>
                  <div>
                    <span className="mono" style={{fontSize:10, letterSpacing:'0.18em', color:'var(--sand)', textTransform:'uppercase'}}>{ev.tag}</span>
                    <h3 style={{fontFamily:'var(--display)', fontSize:30, lineHeight:1.1, marginTop:8}}>{ev.t}</h3>
                  </div>
                  <div className="body" style={{fontSize:14}}>{ev.l}</div>
                  <div>
                    <div className="mono" style={{fontSize:11, color:'var(--sand)', marginBottom:6}}>{ev.spots}/{ev.total} doldu</div>
      ...
      <section id="contrib" className="section">
        <div className="wrap">
          <div className="reveal">
            <SectionHead no="02" eyebrow="Nasıl katkıda bulunulur?"
              title={<>Her yetenek için bir <em>yol</em>.</>}
              ...
              <div key={t} className="card">
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                  <span style={{
                    ...
                    fontFamily:'var(--display)', fontSize:22, color:'var(--crimson)',
                    border:'1px solid rgba(200,16,46,0.30)', borderRadius:6
                  }}>{glyph}</span>
                  <span className="mono" style={{color:'var(--crimson)', fontSize:11}}>№ 0{i+1}</span>
                </div>
                <h3 className="h3" style={{marginTop:22}}>{t}</h3>
                <div className="eyebrow" style={{marginTop:6}}>{sub}</div>
                <p className="body" style={{marginTop:16}}>{b}</p>
                <div style={{marginTop:26}}>
                  <a href="#" className="tlink" style={{fontSize:13}}>Başla →</a>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section" style={{background:'var(--ink-1)', borderTop:'1px solid var(--ink-line)', borderBottom:'1px solid var(--ink-line)'}}>
        <div className="wrap">
          <div className="reveal" style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:16}}>
            <SectionHead no="03" eyebrow="Bu ay öne çıkanlar"
              title={<>En çok <em>katkıda</em> bulunanlar.</>}
            />
            <span className="mono" style={{color:'var(--sand)'}}>Mayıs 2026</span>
          </div>

          <div className="reveal" style={{marginTop:64, border:'1px solid var(--ink-line)', borderRadius:8, overflow:'hidden', background:'var(--ink-2)'}}>
            <div style={{
              display:'grid', gridTemplateColumns:'60px 60px 1.4fr 1fr 1.2fr 120px',
              padding:'16px 24px',
              ...
              <span>№</span><span></span><span>İsim</span><span>Şehir</span><span>Alan</span><span style={{textAlign:'right'}}>Katkı</span>
            </div>
            {tpContributors.map((c, i) => (
              <div key={c[0]} style={{
                ...
                <span style={{
                  ...
                  fontFamily:'var(--display)', fontSize:13, color:'var(--ink)'
                }}>{c[4]}</span>
                ...
                <span className="mono" style={{textAlign:'right', color:'var(--ink)'}}>{c[3]}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section--loose section" style={{position:'relative', overflow:'hidden'}}>
        ...
        <div className="wrap" style={{maxWidth:920, margin:'0 auto', textAlign:'center', position:'relative', zIndex:2}}>
          ...
          <blockquote className="reveal" style={{
            ...
            "Annem 64 yaşında, ilk defa kendi başına bilgisayar
            kullanıyor. <i style={{fontFamily:'var(--serif)', color:'var(--crimson)'}}>Çünkü artık ekrandakini anlıyor.</i>"
          </blockquote>
      ...
      <section id="join" className="section" style={{background:'var(--ink-1)', borderTop:'1px solid var(--ink-line)'}}>
        <div className="wrap" style={{textAlign:'center', maxWidth:760, margin:'0 auto'}}>
          <div className="reveal kicker" style={{justifyContent:'center'}}>
            <span className="glyph"/><span style={{color:'var(--crimson)'}}>04</span>
            <span style={{color:'var(--fg-faint)'}}>·</span><span>Forum</span>
          </div>
          ...
          <p className="lede reveal delay-1" style={{marginInline:'auto', marginTop:24}}>
            Türkçe sorulara Türkçe yanıtlar. Ortalama yanıt süresi: 38 dakika. Topluluğun
            %72'si üniversite öğrencisi ya da yeni başlayan kullanıcılardan oluşuyor.
          </p>
          <div className="reveal delay-2" style={{display:'flex', gap:12, justifyContent:'center', marginTop:40, flexWrap:'wrap'}}>
            <a href="#" className="btn btn-crimson">forum.kapitan.org.tr →</a>
            <a href="#" className="btn btn-line">Discord</a>
            <a href="#" className="btn btn-ghost">IRC #kapitan</a>
          </div>
        </div>
      </section>
    </>
